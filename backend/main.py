...
async def authorize_hubspot_integration(user_id: str, org_id: str):
    # get the URL string from the helper
    url = await authorize_hubspot(user_id, org_id)
    # return JSON that your frontend expects
...
@app.get('/integrations/hubspot/items')  # <<< ADD THIS
async def get_hubspot_items(user_id: str, org_id: str):
    creds_dict = await get_hubspot_credentials(user_id, org_id)
    return await get_items_hubspot(creds_dict)
...
async def load_slack_data_integration(credentials: str = Form(...)):
    creds_dict = json.loads(credentials)  # Convert string to dict
    return await get_items_hubspot(creds_dict)
